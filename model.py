...
class DLRMCustom:
    ...
    def build_model(self):
        ...
        eb_configs = [
            EmbeddingBagConfig(
                ...
                num_embeddings=self.num_embeddings_per_feature[
                    feature_name + "_enc"
                ],  # Already len(v) + 1
    ...
    def evaluate(self, eval_data):
        ...
        if np.isnan(l_preds).any():
            raise ValueError("NaN values detected in predictions during evaluation.")
    ...
    def train(self, train_data, nb_batches):
        ...
        if nb_batches is None or (
            nb_batches is not None and nb_batches < len(train_data.batches)
        ):
            batch_index = np.arange(len(train_data.batches))
        else:
            batch_index = np.random.randint(
                0, len(train_data.batches), nb_batches, replace=False
            )
        ...
        if np.isnan(l_preds).any():
            raise ValueError("NaN values detected in predictions during training.")
    ...
    def train_test(self, train_data, test_data, n_epochs, e_patience, nb_batches=None):
        ...
        try:
            ...
            for epoch in pbar:
                ...
                if (
                    epoch > 0
                    and abs(scores[epoch]["auc_test"] - scores[epoch - 1]["auc_test"])
                    < 0.001
                ):
                    k += 1
                if k > e_patience:
...
def plot_results(results):
    ...
    fig.add_trace(
        go.Scatter(x=results.epoch, y=results.loss_train, name="loss_train"),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(x=results.epoch, y=results.loss_test, name="loss_test"),
        secondary_y=False,
    )
    # Add training and validation AUC traces to the figure
    fig.add_trace(
        go.Scatter(x=results.epoch, y=results.auc_train, name="auc_train"),
        secondary_y=True,
    )
    fig.add_trace(
        go.Scatter(x=results.epoch, y=results.auc_test, name="auc_test"),
        secondary_y=True,
    )
...
def optimizer_with_params(adagrad, learning_rate, eps=None):
    ...
    if adagrad:
        ...
        return lambda params: torch.optim.Adagrad(params, lr=learning_rate, eps=eps)
    else:
